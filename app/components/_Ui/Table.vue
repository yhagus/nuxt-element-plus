...
<template>
  <div>
    <el-table :size="size ?? 'small'" :data="data" :span-method="spanMethod" :row-class-name="rowClassName" :stripe="stripe" :border="border" @row-click="onRowClick" @selection-change="handleSelectionChange">
      <template #empty>
        <div class="flex items-center justify-center p-10 text-gray-500">
          <!-- Loading Spinner -->
          <div v-if="loading" role="status" class="flex items-center space-x-3">
            <svg aria-hidden="true" class="h-6 w-6 animate-spin fill-blue-600 text-gray-200 dark:text-gray-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">

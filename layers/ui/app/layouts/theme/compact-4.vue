...
<template>
  <div class="compact-layout compact-layout--indigo">
    <aside class="compact-layout__sidebar" :style="{ width: `${sidebarWidth}px` }">
      <ThemeCompactFourLayoutSidebar />
    </aside>
    ...
    <div class="compact-layout__content" :style="{ paddingLeft: `${isDesktop ? sidebarWidth : 0}px` }">
      <header class="compact-layout__header">
        <ThemeCompactFourLayoutHeader @toggle-sidebar="toggleSidebar" />
      </header>
      <main class="compact-layout__main">
        <div class="compact-layout__stats">
          <ThemeCompactFourUiMiniStat label="Runs" value="63" />
          <ThemeCompactFourUiMiniStat label="Fail" value="1" />
          <ThemeCompactFourUiMiniStat label="Sync" value="Ok" />
        </div>
        <ThemeCompactFourUiDensePanel title="Compact Workspace">
          <slot />
        </ThemeCompactFourUiDensePanel>
      </main>
      <ThemeCompactFourLayoutFooter />
    </div>
  </div>
</template>

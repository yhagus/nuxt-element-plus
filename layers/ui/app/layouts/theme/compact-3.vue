...
<template>
  <div class="compact-layout compact-layout--rose">
    <aside class="compact-layout__sidebar" :style="{ width: `${sidebarWidth}px` }">
      <ThemeCompactThreeLayoutSidebar />
    </aside>
    ...
    <div class="compact-layout__content" :style="{ paddingLeft: `${isDesktop ? sidebarWidth : 0}px` }">
      <header class="compact-layout__header">
        <ThemeCompactThreeLayoutHeader @toggle-sidebar="toggleSidebar" />
      </header>
      <main class="compact-layout__main">
        <div class="compact-layout__stats">
          <ThemeCompactThreeUiMiniStat label="Leads" value="94" />
          <ThemeCompactThreeUiMiniStat label="Calls" value="12" />
          <ThemeCompactThreeUiMiniStat label="Won" value="4" />
        </div>
        <ThemeCompactThreeUiDensePanel title="Compact Workspace">
          <slot />
        </ThemeCompactThreeUiDensePanel>
      </main>
      <ThemeCompactThreeLayoutFooter />
    </div>
  </div>
</template>
